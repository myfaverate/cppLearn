...
void create_patch(const char* old_path, const char* new_path, const char* patch_path) {
    std::vector<uint8_t> old_data = load_file(old_path);
    std::vector<uint8_t> new_data = load_file(new_path);

    FILE* pf = fopen(patch_path, "wb");
